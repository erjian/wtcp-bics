...
(SELECT REPLACE(s.file_url,'\\','/') vrImage FROM t_bic_material s WHERE TRIM(s.file_identify)='vr' AND s.principal_id = t.id) vrImageUrl,	'' videoResources,
...
from t_bic_extend_tags t LEFT JOIN t_bic_extend b on t.principal_id = b.id
where t.principal_id in(select a.id from t_bic_extend a  ) GROUP BY t.principal_id;
