...
public class HallController extends BaseController {
    ...
    public ResponseMessage findByPage(@RequestParam(value = "page", defaultValue = "0") Integer page,
        ...
        return hallService.findByPage(page, size, filter);
    }
    ...
    public ResponseMessage findByList(HttpServletRequest request) {
        Map<String, Object> filter = RequestUtil.getParameters(request);
        return hallService.findByList(filter);
    }
    ...
    public ResponseMessage detail(@PathVariable("id") String id) {
        HallEntity hallEntity = hallService.selectByPrimaryKey(id);
        if (hallEntity == null) {
            return ResponseMessage.validFailResponse().setMsg("数据不存在");
        }
        return ResponseMessage.defaultResponse().setData(hallEntity);
    }
    ...
    public ResponseMessage delete(@PathVariable("id") String id) {
        return hallService.deleteByPrimaryKey(id);
    }
    ...
    public ResponseMessage save(@RequestBody EntityTagsModel<HallEntity> hallModel, BindingResult bindingResult) throws Exception {
        ...
        return hallService.insert(hallModel, getCurrentUser(), ruleId, appCode);
    }
    ...
    public ResponseMessage changeStatus(@RequestParam String id, @RequestParam Integer status) throws Exception {
        return hallService.changeStatus(id, status, getCurrentUser().getUsername());
    }
    ...
    public ResponseMessage dataBind(@RequestBody @Valid DataBindModel model, BindingResult bindingResult) throws Exception {
        ...
        return hallService.dataBind(getCurrentUser().getUsername(), model);
    }
    ...
    public ResponseMessage existByTitle(@RequestParam String title, String id) {
        return hallService.existsByTitleAndIdNot(title, id != null ? id : "-1");
    }
